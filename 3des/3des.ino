...
bool notifiedTVOC = false;
bool notifiedeCO2 = false;
...
// Read data from sensors and handle LED indicators and notifications
void readSensorData() {
    if (ccs811.available()) {
        if (!ccs811.readData()) {
            ...
            if (humidity > 50) {
                ...
                if (!notifiedHumidity) {
                    Blynk.logEvent("humidity_high", String("Humidity is high: ") + humidity + String("%"));
                    notifiedHumidity = true;
                }
            } else {
                digitalWrite(BLUE_LED_PIN, LOW);
                notifiedHumidity = false;
            }

            // Check for TVOC high
            if (tvoc > 1000) {
                if (!notifiedTVOC) {
                    Blynk.logEvent("tvoc_high", String("TVOC is high: ") + tvoc + String("PPB"));
                    notifiedTVOC = true;
                }
            } else {
                notifiedTVOC = false;
            ...
            if (eco2 > 1500) {
                if (!notifiedeCO2) {
                    Blynk.logEvent("ec02_high", String("eCO2 is high: ") + eco2 + String("PPM"));
                    notifiedeCO2 = true;
                }
            } else {
                notifiedeCO2 = false;
            ...
            // Check for temperature high
            if (temperatureC > 30) { // Replace 30 with your threshold
                if (!notifiedTemp) {
                    Blynk.logEvent("temp_high", String("Temp is high: ") + temperatureF + String("F/") + temperatureC + String("C"));
                    notifiedTemp = true;
                }
            } else {
                notifiedTemp = false;
